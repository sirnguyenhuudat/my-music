$(document).ready(function () {
    $(window).load(window.location.href, function (response, status, xhr) {
        if (response !== '') {
            var result = JSON.parse(response);
            playTrack(result.image, result.name_track, result.artist_name, result.path);
        }
    });
});
function playTrack(image, name_track, artist_name, path) {
