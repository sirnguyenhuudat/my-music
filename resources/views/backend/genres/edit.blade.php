                    ...
                    <div class="col-lg-6 offset-lg-3">
                        <div class="card">
                            <div class="card-header">
                                {!! trans('backend_genre.label_form_update') !!}
                            </div>
                            <div class="card-body card-block">
                                <form action="{{ route('backend.genres.update', $genre->id) }}" method="post" enctype="multipart/form-data">
                                    @csrf
                                    @method('put')
                                    <div class="form-group {{ $errors->has('name') ? 'has-warning' : '' }}">
                                        ...
                                        <input type="text" class="form-control-success form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" name="name" value="{{ $genre->name }}">
                                    </div>
                                    @if($genre->picture)
                                    <div class="form-group">
                                        <label class="form-control-label">{{ trans('backend_genre.old_image') }}</label>
                                    ...
                                    <div class="form-group {{ $errors->has('name') ? 'has-warning' : 'has-success' }}">
                                        ...
                                        <textarea name="description" id="genreEditor" class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" cols="30" rows="10">{{ $genre->description }}</textarea>
                                    </div>
                                    <div class="form-group">
